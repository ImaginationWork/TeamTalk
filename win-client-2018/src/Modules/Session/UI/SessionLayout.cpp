...
void SessionLayout::OnWindowInitialized()
{
    if (isInitialzied){
        LOG__(ERR, _T("SessionLayout::OnWindowInitialized isInitialzied"));
        throw;
    }
    ...
    auto username = dynamic_cast<CTextUI*>(m_pManager->FindSubControlByName(this, _T("username")));
    username->SetText(pSessionInfo->getName());
...
void SessionLayout::DocmentComplete(IDispatch *pDisp, VARIANT *&url) //打开会话已经，等浏览器初始化完成，加载未读消息
{
...
SessionContainer::SessionContainer(CPaintManagerUI& paint_manager)
    :paint_manager(paint_manager)
{
}

void SessionContainer::AddSession(const std::string &sId)
{
    for (auto &item : layoutList){
        auto sitem = dynamic_cast<SessionLayout*>(item);
        if (sitem->m_sId == sId){
            SelectItem(item);
            return;
        }
    ...
    auto root = builder.Create(_T("MainWindow\\SessionLayout.xml"), (UINT)0, this, &paint_manager);
    if (nullptr == root){
        //throw new exception
        LOG__(ERR, _T("root==nullptr"));
    }
    Add(root);
    layoutList.push_back(root);
    //curr->SetManager(&paint_manager, this, true);
    curr->OnWindowInitialized();
    SelectItem(root);
#else
    auto text = new CTextUI();
    static int index = 0;
    text->SetText(std::to_wstring(index++).c_str());
    Add(text);
    SelectItem(text);
#endif
}
...
bool SessionContainer::haveSessionLayoutBySId(const std::string &sId)
{
    for (auto &item : layoutList){
        auto sitem = dynamic_cast<SessionLayout*>(item);
        if (sitem->m_sId == sId){
...
bool SessionContainer::UpdateRunTimeMsg(const std::string &sId)
{
    for (auto &item : layoutList){
        auto sitem = dynamic_cast<SessionLayout*>(item);
        if (sitem->m_sId == sId){
            sitem->UpdateRunTimeMsg();
            return true;
        }
    }
...
bool SessionContainer::DoDisplayHistoryMsgToIE(const std::string &sid, std::vector<MessageEntity>& msgList, IN const BOOL scrollBottom)
{
    for (auto &item : layoutList){
        auto session = dynamic_cast<SessionLayout*>(item);
        if (session->m_sId == sid){
            session->DoDisplayHistoryMsgToIE(msgList, scrollBottom);
            return true;
        }
    }
    return false;
...
DuiLib::CControlUI* SessionContainer::CreateControl(LPCTSTR pstrClass)
{
    if (_tcsicmp(pstrClass, _T("SessionLayout")) == 0)
    {
        auto m_pSessionLayout = new SessionLayout(tempSID, paint_manager);
        curr = m_pSessionLayout;

        return m_pSessionLayout;
    }
    else if (_tcsicmp(pstrClass, _T("UIIMEdit")) == 0)
    {
        return new UIIMEdit();
    }
    return NULL;
