...
,delay_number_(0)
,delay_left_(0)
,level_expand_image_(_T("<i MainWindow\\list_icon_b.png>"))
,level_collapse_image_(_T("<i MainWindow\\list_icon_a.png>"))
...
Node* UIIMList::AddNode(const IMListItemInfo& item, Node* parent, int index)
{
	...
	if (!m_dlgBuilder.GetMarkup()->IsValid())
	{
		...
		pListElement = static_cast<CListContainerElementUI*>(m_dlgBuilder.Create(_T("MainWindow\\recentlyListItem.xml"), (UINT)0, NULL, &paint_manager_));
	}
	else
	{
