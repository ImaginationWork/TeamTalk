...
namespace{
    SessionContainer *sessionContainer = nullptr;
}
	...
	IN LPCTSTR pstrClass,IN DuiLib::CPaintManagerUI& paintManager)
{
    ...
    else if (_tcsicmp(pstrClass, _T("SessionContainer")) == 0) {
        sessionContainer = new SessionContainer(paintManager);
...
bool SessionModule_Impl::onUpdateRunTimeMsg(const std::string &sid) const
{
    return sessionContainer->UpdateRunTimeMsg(sid);
}

void SessionModule_Impl::showSession(const std::string &sid) const
{
    sessionContainer->AddSession(sid);
}

bool SessionModule_Impl::haveSessionLayoutBySId(const std::string &sid) const
{
    return sessionContainer->haveSessionLayoutBySId(sid);
}

bool SessionModule_Impl::DoDisplayHistoryMsgToIE(const std::string &sid, std::vector<MessageEntity>& msgList, IN const BOOL scrollBottom) const
{
    return sessionContainer->DoDisplayHistoryMsgToIE(sid, msgList, scrollBottom);
}
