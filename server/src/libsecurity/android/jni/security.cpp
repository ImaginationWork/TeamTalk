...
extern "C" {
    ...
    jbyteArray Java_com_mogujie_tt_Security_EncryptMsg2(JNIEnv* env, jobject obj, jbyteArray inputArray)
    {
        const char *pInData = (const char *) env->GetByteArrayElements(inputArray, 0);
        uint32_t nInLen = env->GetArrayLength(inputArray);
        ...
        unsigned char* pData = (unsigned char*) calloc(nEncryptLen, 1);
        memcpy(pData, pInData, nInLen);
        env->ReleaseByteArrayElements(inputArray,(jbyte*)pInData,JNI_ABORT);
    ...
    jbyteArray Java_com_mogujie_tt_Security_DecryptMsg(JNIEnv* env, jobject obj, jstring jstr)
    {
        ...
        const char *key = "12345678901234567890123456789012";
        AES_set_decrypt_key((const unsigned char*) key, 256, &aesKey);   
        for (uint32_t i = 0; i < nBlocks; i++) {
    ...
    jbyteArray Java_com_mogujie_tt_Security_DecryptMsg2(JNIEnv* env, jobject obj, jbyteArray inputArray)
    {
        const char *pInData = (const char *)env->GetByteArrayElements(inputArray, NULL);  
        uint32_t nInLen = env->GetArrayLength(inputArray);
        string strInData(pInData, nInLen);
        env->ReleaseByteArrayElements(inputArray,(jbyte*)pInData,JNI_ABORT);
        std::string strResult = base64_decode(strInData);
